...
const SmallPJ = (props)=>{
    return(
        ...
        <div className={props.twobytwo ? "pj-small pj-twobytwo " : "pj-small "} style={props.twobytwo && {width:"calc(50% - 30px)",flexDirection:"column"}}>
           ...
           {!props.twobytwo && <div className="pj-small-pic " style={props.picConHeight ? {height:`${props.picConHeight}`} : props.widthbg && {background:`url("/img/smallpj/${props.pjimg}.jpg")`,backgroundSize:"cover"}}>
           
                <img src={`/img/smallpj/${props.pjimg}.jpg`} style={props.widthbg && {opacity:0} }/>
                
            </div>}
            </SRLWrapper>
            <div className="pj-small-description" style={props.twobytwo && {width:"100%",padding:"0"}}>
                <p className={props.widthbg ? `pj-small-description-text widthbg` : 'pj-small-description-text'} style={props.twobytwo ? {fontSize:"0.75rem"} : {boxSizing:'border-box',paddingLeft:"10px"}}>{props.children}</p>
                <div className="pj-small-bottom">
                <div className="pj-web pj-small-web">
    <a className="pj-web-text twobytwotext" href={`https://${props.url}`} target="blank">{props.url}</a>
                </div>
